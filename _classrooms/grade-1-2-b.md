---
layout: classroom
title: Grade 1/2 Class B
grade: "1-2"
teachers:
  - Levita Morados
color: blue
order: 3
room: Classroom 2 - Grade 1/2
students: 10
door_photo: /assets/images/classrooms/grade-1-2-b/door-photo.jpg
hero_image: /assets/images/classrooms/grade-1-2-b/hero-classroom-reading-corner.jpg
newsletter_folder: grade-1-2-b
show_daily_schedule: true
show_photo_gallery: false
teacher_photos:
  - name: Levita Morados
    photo: /assets/images/classrooms/grade-1-2-b/levita-morados-headshot.jpg
    bio:
      - "Ms. Levita — many students call her \"Ms. Vita\" for short — is originally from the Philippines, where she completed her education before continuing her studies to pursue higher education in teaching. Her passion for teaching grew through that journey, driven by a desire to help students hone their skills and let them learn at their own pace."
      - "She is pleased to be part of the HCMS school community and looks forward to this new school year, creating a calm, welcoming, and organized classroom where every student feels welcomed, encouraged, and supported each day."
teacher_emails:
  - name: Levita Morados
    email: levitamae.morados@gmail.com
subjects:
  - name: Oji-Cree Language
    description: Taught by Ms. Brenda, focusing on vocabulary, greetings, traditional stories, songs, and cultural activities.
    icon: message
  - name: Language Arts
    description: Phonics, sight words, reading comprehension, fluency, and daily writing activities including creative and informational writing.
    icon: book
  - name: Mathematics
    description: Number sense, addition/subtraction, patterns, measurement, money, time, data collection, and geometry through hands-on exploration.
    icon: calculator
  - name: Science & Technology
    description: Inquiry and experimentation exploring living things, life cycles, material properties, seasonal changes, and simple machines.
    icon: flask
  - name: Social Studies
    description: Learning about families, communities, traditions, roles and responsibilities, and local history and geography.
    icon: globe
  - name: Health & Physical Education
    description: Motor skills, healthy eating, personal safety, teamwork, and mental/emotional well-being through daily activities.
    icon: heart
  - name: The Arts
    description: Visual arts, drama, music, and movement woven throughout the day to build creativity and self-expression.
    icon: palette
schedule:
  monday:
    - time: "9:00 - 9:30"
      activity: "Arrival Time & Morning Work"
    - time: "9:30 - 9:45"
      activity: "Breakfast"
    - time: "9:45 - 10:00"
      activity: "Art"
    - time: "10:00 - 10:15"
      activity: "Recess"
    - time: "10:15 - 10:30"
      activity: "P.E. (Gym)"
    - time: "10:45 - 11:00"
      activity: "Math"
    - time: "11:00 - 11:30"
      activity: "Native Language"
      details: "Ms. Brenda"
    - time: "11:30 - 12:00"
      activity: "Prepare & Send Students to the Bus"
    - time: "12:00 - 1:00"
      activity: "Lunch"
      highlight: true
    - time: "1:00 - 1:15"
      activity: "Printing / Writing"
    - time: "1:15 - 1:30"
      activity: "Literacy"
    - time: "1:45 - 2:00"
      activity: "Recess"
    - time: "2:00 - 2:15"
      activity: "Snack Time"
    - time: "2:15 - 2:30"
      activity: "Music"
      details: "Mr. Felipe"
    - time: "2:30 - 2:45"
      activity: "Science"
    - time: "2:45 - 3:00"
      activity: "Choice Time, Story Time & Clean Up"
    - time: "3:00 - 3:15"
      activity: "Dismissal / After School Program"
  tuesday:
    - time: "9:00 - 9:30"
      activity: "Arrival Time & Morning Work"
    - time: "9:30 - 9:45"
      activity: "Breakfast"
    - time: "9:45 - 10:00"
      activity: "Art"
    - time: "10:00 - 10:15"
      activity: "Recess"
    - time: "10:15 - 10:30"
      activity: "P.E. (Gym)"
    - time: "10:30 - 10:45"
      activity: "Literacy"
    - time: "11:00 - 11:15"
      activity: "Social Studies / Science"
    - time: "11:30 - 12:00"
      activity: "Prepare & Send Students to the Bus"
    - time: "12:00 - 1:00"
      activity: "Lunch"
      highlight: true
    - time: "1:00 - 1:15"
      activity: "Printing / Writing"
    - time: "1:15 - 1:30"
      activity: "Science"
    - time: "1:30 - 1:45"
      activity: "Math"
    - time: "1:45 - 2:00"
      activity: "Recess"
    - time: "2:00 - 2:15"
      activity: "Snack Time"
    - time: "2:15 - 2:30"
      activity: "Native Language"
      details: "Ms. Brenda"
    - time: "2:45 - 3:00"
      activity: "Choice Time, Story Time & Clean Up"
    - time: "3:00 - 3:15"
      activity: "Dismissal / After School Program"
  wednesday:
    - time: "9:00 - 9:30"
      activity: "Arrival Time & Morning Work"
    - time: "9:30 - 9:45"
      activity: "Breakfast"
    - time: "9:45 - 10:00"
      activity: "Art"
    - time: "10:00 - 10:15"
      activity: "Recess"
    - time: "10:15 - 10:30"
      activity: "P.E. (Gym)"
    - time: "10:45 - 11:00"
      activity: "Math"
    - time: "11:00 - 11:30"
      activity: "Native Language"
      details: "Ms. Brenda"
    - time: "11:30 - 12:00"
      activity: "Prepare & Send Students to the Bus"
    - time: "12:00 - 1:00"
      activity: "Lunch"
      highlight: true
    - time: "1:00 - 1:15"
      activity: "Printing / Writing"
    - time: "1:15 - 1:30"
      activity: "Music"
      details: "Mr. Felipe"
    - time: "1:30 - 1:45"
      activity: "Catch-Up Time"
      details: "Lexia, IXL, Symphony Math"
    - time: "1:45 - 2:00"
      activity: "Recess"
    - time: "2:00 - 2:15"
      activity: "Snack Time"
    - time: "2:15 - 2:30"
      activity: "Social Studies"
    - time: "2:30 - 2:45"
      activity: "Literacy"
    - time: "2:45 - 3:00"
      activity: "Choice Time, Story Time & Clean Up"
    - time: "3:00 - 3:15"
      activity: "Dismissal / After School Program"
  thursday:
    - time: "9:00 - 9:30"
      activity: "Arrival Time & Morning Work"
    - time: "9:30 - 9:45"
      activity: "Breakfast"
    - time: "9:45 - 10:00"
      activity: "Art"
    - time: "10:00 - 10:15"
      activity: "Recess"
    - time: "10:15 - 10:30"
      activity: "P.E. (Gym)"
    - time: "10:30 - 10:45"
      activity: "Literacy"
    - time: "11:00 - 11:15"
      activity: "Social Studies / Science"
    - time: "11:30 - 12:00"
      activity: "Prepare & Send Students to the Bus"
    - time: "12:00 - 1:00"
      activity: "Lunch"
      highlight: true
    - time: "1:00 - 1:15"
      activity: "Printing / Writing"
    - time: "1:15 - 1:30"
      activity: "Catch-Up Time"
      details: "Lexia, IXL, Symphony Math"
    - time: "1:30 - 1:45"
      activity: "Math"
    - time: "1:45 - 2:00"
      activity: "Recess"
    - time: "2:00 - 2:15"
      activity: "Snack Time"
    - time: "2:15 - 2:30"
      activity: "Native Language"
      details: "Ms. Brenda"
    - time: "2:45 - 3:00"
      activity: "Choice Time, Story Time & Clean Up"
    - time: "3:00 - 3:15"
      activity: "Dismissal / After School Program"
  friday:
    - time: "9:00 - 9:30"
      activity: "Arrival Time & Morning Work"
    - time: "9:30 - 9:45"
      activity: "Breakfast"
    - time: "9:45 - 10:00"
      activity: "Art"
    - time: "10:00 - 10:15"
      activity: "Recess"
    - time: "10:15 - 10:30"
      activity: "P.E. (Gym)"
    - time: "10:45 - 11:00"
      activity: "Math"
    - time: "11:00 - 11:30"
      activity: "Native Language"
      details: "Ms. Brenda"
    - time: "11:30 - 12:00"
      activity: "Prepare & Send Students to the Bus"
    - time: "12:00 - 1:00"
      activity: "Lunch"
      highlight: true
    - time: "1:00 - 1:15"
      activity: "Printing / Writing"
    - time: "1:15 - 1:30"
      activity: "Literacy"
    - time: "1:45 - 2:00"
      activity: "Recess"
    - time: "2:00 - 2:15"
      activity: "Snack Time"
    - time: "2:15 - 2:30"
      activity: "Science"
    - time: "2:30 - 2:45"
      activity: "Catch-Up Time"
      details: "Lexia, IXL, Symphony Math"
    - time: "2:45 - 3:00"
      activity: "Choice Time, Story Time & Clean Up"
    - time: "3:00 - 3:15"
      activity: "Dismissal / After School Program"
---

Boozhoo! Welcome to our Grade 1/2 classroom at Henry Coaster Memorial School! This new school year, I am so excited to be your child's teacher.

To our dear parents and guardians, I am honored for the opportunity to teach your children. Our classroom is a place where we will explore new things, create amazing memories in the class, have a wonderful time learning and growing together, and where they will feel safe. Let's get ready for an incredible year!

## Our Classroom Philosophy

With dedication to fostering a love of learning in young children, our classroom is a positive environment where each child feels supported and valued. Students are encouraged to learn at their own pace and track, with different learning styles and instructional techniques catered to their individual needs. Every subject is taught through an engaging, hands-on curriculum designed to spark active learning and curiosity.

In Grade 1/2, we integrate land-based learning and incorporate the **Seven Sacred Teachings** (also known as the Seven Grandfather Teachings) into our daily practice, helping students learn and understand what it means to be a responsible member of the community. Every child can achieve with encouragement and the right guidance — in our classroom, we prioritize helping children build their confidence, resilience, respect for others, and social-emotional development.

## How Families Can Support Learning

Families play an essential role in supporting children's learning and development. Here are some ways you can help your child succeed:

**Reading Together** - Read books sent home together regularly. Even 10-15 minutes each day makes a big difference in building literacy skills and a love of reading.

**Using Online Learning Tools** - Encourage your child to use Lexia, IXL, and Symphony Math at home. These programs provide personalized practice that supports classroom learning.

**Talking About Learning** - Ask your child about what they learned at school. Celebrate their effort and progress, not just their results.

**Practicing Oji-Cree** - Use Oji-Cree words and phrases at home when possible. Sharing family stories, traditions, and experiences helps children feel proud of who they are.

**Staying Connected** - Reach out with questions or concerns. We are here to support you and your child!

Thank you for entrusting me with your children. I look forward to partnering with you in their education!

**Miigwech!**
*Ms. Levita*
